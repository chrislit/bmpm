...
for ($ln = 0; $ln < count($lines); $ln++) {
    ...
    if ($match == "approx") {
        $result2 = Phonetic_UTF8($name, $rules[LanguageIndexFromCode($languageCode2, $languages)], $approxCommon, $approx[LanguageIndexFromCode($languageCode2, $languages)], $languageCode2);
    } else {
        $result2 = Phonetic_UTF8($name, $rules[LanguageIndexFromCode($languageCode2, $languages)], $exactCommon, $exact[LanguageIndexFromCode($languageCode2, $languages)], $languageCode2);
    }
    $numbers2 = PhoneticNumbers($result2);

    #$soundex = soundx_name($name);
    fputs($handle, "$name,$numbers2\n");
}
echo "Done<br>";
?> 
